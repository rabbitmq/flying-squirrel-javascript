...
if (this.io === undefined) {
    if (window.console) {console.error('socket.io is missing');}
}
...
(function() {
	 ...
	 var that = this;
         this.uid = (String()+Math.random()).substr(2, 8);
	 ...
	 'publish': function(channel, body) {
             this._send({channel:channel, body:body});
	 },
	 'subscribe': function(channel, callback) {
	     this.channels[channel] = callback;
	 },
         'request': function(channel, question, callback) {
             // It's illegal to send two requests on the same channel
             // at the same time. Request should be treated as a
             // synchronous thing.
             var that = this;
             that.subscribe(channel, function(answer, channel, msgobj) {
                                callback(answer, channel, msgobj);
                                that.subscribe(channel, function() {
                                                   that._error_and_close(
                                                       'unexpected reply');
                                               });
                            });
         ...
         'serve': function(channel, callback) {
             var that = this;
             that.subscribe(channel, function(msg, channel, msgobj) {
                                var replyto = msgobj['reply-to'];
                                callback(msg, channel, msgobj, function(answer) {
                                             that._send({'channel': channel,
                                                         'reply-to': replyto,
                                                         'body': answer});
                                         });
                            });
	 ...
	 '_state_disconnected': function (reason, message) {
         ...
         '_state_handshake': function (reason) {
             ...
             this._on_disconnect = function () {
                 ...
                 that.socket.disconnect();
                 that._next('_state_disconnected', 'refused', 'disconnected');
             };
